...
model_cum_smoke<- glm(alc_ever ~ cum_smoke_preg, data = new_df, family = binomial(link = "logit"))
summary(model_cum_smoke)

#create plot showing the probability predictions 
new_cum_data <- data.frame(cum_smoke_preg = new_df$cum_smoke_preg)

# Predict probabilities using the model
new_cum_data$Predicted_Probabilities<- predict(model_cum_smoke, newdata = new_cum_data, type = "response")
